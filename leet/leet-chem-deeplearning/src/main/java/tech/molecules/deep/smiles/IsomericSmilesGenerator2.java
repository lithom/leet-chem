...
public class IsomericSmilesGenerator2 {
    ...
    public static String createSmiles(StereoMolecule mol) {
        return new IsomericSmilesGenerator2(mol, 0).getSmiles();
    }
    ...
    public static String createSmarts(StereoMolecule mol) {
        return new IsomericSmilesGenerator2(mol, MODE_CREATE_SMARTS,null).getSmiles();
    }
    ...
    public static String createReactionSmiles(Reaction rxn, int mode) {
        ...
        for (int i=0; i<rxn.getProducts(); i++) {
            if (i != 0)
                sb.append('.');
            sb.append(new IsomericSmilesGenerator2(rxn.getProduct(i), mode).getSmiles());
        }
        return sb.toString();
    }
     ...
     * @param mol
     */
    public IsomericSmilesGenerator2(StereoMolecule mol) {
        this(mol, false);
    }

    public IsomericSmilesGenerator2(StereoMolecule mol, int mode) {
        this(mol, mode,null);
    }
     ...
     */
    public IsomericSmilesGenerator2(StereoMolecule mol, int mode, Random ri) {
        mMol = mol;
        mMode = mode;
        this.random = ri;
    }
     ...
     */
    @Deprecated
    public IsomericSmilesGenerator2(StereoMolecule mol, boolean includeAtomMapping) {
        this(mol, includeAtomMapping ? MODE_INCLUDE_MAPPING : 0,null);
    }
    ...
    private int findUnusedStartAtom() {
        ...
        if(this.random!=null) {
            ...
            for (int atom=0; atom<mMol.getAtoms(); atom++) {
                if (!mAtomUsed[atom]) {
                    all_used = false; break;
                }
            }
            if(all_used) {return -1;}
            while(true) {
                int ni = random.nextInt(mMol.getAtoms());
                if(!mAtomUsed[ni]) {return ni;}
            }
        }
