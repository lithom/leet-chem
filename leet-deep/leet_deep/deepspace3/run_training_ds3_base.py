from leet_deep.canonicalsmiles import Seq2SeqModel_5, Seq2SeqModel_6
import torch
import time
import sys
import json
...
def create_dataset(conf : ConfigurationLocalData ):
    ...
    dataset = MoleculeDataset_DS3_A(conf.input_file,False,8,selected_batches=None)
...
def create_model(model_dim, model_layers):
    # Hyperparameters
    ...
    print(f'Init Model, dim={model_dim}, nhead={nhead}, nlayers={model_layers}, dim_ff={dim_feedforward}')
    # to change for later: dim_expansion_out_1 should then be 8..

    expansions = []
    # ( head type, (shape_of_elements_out) )
    # expansions have a specific "head" and then the extra dimensions to 32,
    # i.e. the actual output dimension is
    #
    #expansions.append( (2,(32,32,8,2)) ) # adj matrix
    expansions.append( ( 'c2_a' , [32,2,2] ) ) # adj matrix
    expansions.append( ( 'cx_a' , [17,6] ) ) # chem properties2 (0..5) for the counts / classes.
    expansions.append( ( 'n_a' , [32,1] ) ) # dist matrix (numerical interpretation..)
    expansions.append( ( 'c2_a' , [32, 2] ) )  # shared plane constraints
    #expansions.append( (16,(32,13,4,16)) ) # atom counts

    model = Seq2SeqModel_6(sequence_length_in=64,sequence_length_out=32,vocab_size_in=44,expansions_list=expansions,model_dim=model_dim, num_layers=model_layers) # (B)
    return model


def train(model, dataloader, optimizer, criterion_adj, criterion_cp, criterion_distm, criterion_plane, device, num_batches_per_minibatch):
    model.train()
    total_loss_adj = 0
    total_loss_cp = 0
    total_loss_distm = 0
    ...
    for i, batch in enumerate(dataloader):
        ...
        inputs, targets_adj_matrix, targets_chem_prop = batch['smile_enc'].to(device), batch['adj_matrices'].to(device), batch['atom_properties2'].to(device)
        targets_distmatrix , targets_planeconstr = batch['distance_matrices'].to(device) , batch['plane_rule_constraints'].to(device)
        matrix_mask = batch['left_upper_mask'].to(device)

        # Perform forward pass
        # outputs, outputs_2 = model(inputs)
        outputs_all = model(inputs, inputs)

        output_adj_matrix  = outputs_all[2+0]
        output_chem_prop   = outputs_all[2+1]
        output_distmatrix  = outputs_all[2+2]
        output_planeconstr = outputs_all[2+3]
        ...
        #targets_dm = targets_dm
        #targets_planeconstr = targets_planeconstr
        ...
        targets_chem_prop_permuted_clipped = torch.clamp( targets_chem_prop_permuted, 0,5 ).to(torch.int64)

        output_planeconstr_permuted = output_planeconstr.permute(0, 3, 1, 2)
        targets_planeconstr_clamped = torch.clamp( targets_planeconstr ,0,1).to(torch.int64)
        output_distmatrix_squeezed = torch.squeeze( output_distmatrix )
        ...
        loss_planeconstr = criterion_plane(output_planeconstr_permuted, targets_planeconstr_clamped)
        loss_distm = criterion_distm( output_distmatrix_squeezed , torch.log( targets_distmatrix + 1.0 ) )

        #loss_atom_types  = criterion_at(output_atom_types_permuted_clipped, targets_atom_types_permuted_clipped )
        loss_total = loss_adj_matrix + loss_chem_prop + loss_distm + loss_planeconstr #+ loss_atom_types
        # Perform backward pass and optimization
        loss_total.backward()

        if (i+1) % conf.optim_batches_per_minibatch == 0:
            optimizer.step()
            optimizer.zero_grad()
        ...
        total_loss_plane += loss_planeconstr.item()
        total_loss_distm += loss_distm.item()
        #total_loss_at  += loss_atom_types.item()
        print(f"loss_adj: {loss_adj_matrix.item()} , loss_cp: {loss_chem_prop.item()} , loss_dm: {loss_distm.item()} , loss_plane: {loss_planeconstr.item()}")

    print(f"time training: {time.time()-ts_train_a}")
    print(f"loss_adj: {total_loss_adj} , loss_cp: {total_loss_cp} , loss_dm: {total_loss_distm} , loss_plane: {total_loss_plane}")
    return ( (loss_total) / len(dataloader) , (total_loss_adj) / len(dataloader) , (total_loss_cp) / len(dataloader) )

def validate(model, dataloader, criterion_adj, criterion_cp, criterion_distm, driterion_plane, device):
    model.eval()
    total_loss_adj = 0
    total_loss_cp  = 0
    total_loss_distm = 0
    ...
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            output_distmatrix = outputs_all[3]
            output_planeconstr = outputs_all[4]
            ...
            # total_loss_at  += loss_atom_types.item()
            print(
                f"loss_adj: {loss_adj_matrix.item()} , loss_cp: {loss_chem_prop.item()} , loss_dm: {loss_distm.item()} , loss_plane: {loss_planeconstr.item()}")
...
if __name__ == "__main__":
    ...
    model = create_model(conf.model_dim,conf.model_layers)
    ...
    criterion_adj = nn.CrossEntropyLoss(weight=torch.tensor([0.1,0.9],device=conf.device))
    criterion_at  = nn.CrossEntropyLoss(weight=torch.tensor([0.2,0.9,0.9,0.9,0.9,0.9],device=conf.device))
    criterion_distm = nn.SmoothL1Loss() #nn.SmoothL1Loss()
    criterion_plane = nn.CrossEntropyLoss(weight=torch.tensor([0.1,0.9],device=conf.device))# weight=torch.tensor([0.9,0.1]))
    ...
    for epoch in range(num_epochs):
        train_loss = train(model, train_loader, optimizer, criterion_adj, criterion_at, criterion_distm, criterion_plane, device, conf.optim_batches_per_minibatch)
        val_loss = validate(model, val_loader, criterion_adj,criterion_at, criterion_distm, criterion_plane, device)
        scheduler.step()
