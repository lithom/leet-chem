...
def train(model, dataloader, optimizer, criterion, device):
    ...
    for batch in dataloader:
        ...
        inputs, targets, targets_2 = batch[0].to(device), batch[1].to(device), batch[2].to(device)
        ...
        loss = criterion(outputs_2, targets_2) #criterion(outputs,targets) + 4 * criterion(outputs_2, targets_2)
...
def validate(model, dataloader, criterion, device, epoch):
    ...
    return ( (total_loss_2) / len(dataloader) , correct_preds)
...
dataset = MoleculeDataset_Int(conf.input_file_SmilesEnc, conf.input_file_DM, conf.input_file_cheminfo)#MoleculeDataset_Int('C:/dev7/leet/smi64_atoms32_alphabet50_SmilesEncoded.npy', 'C:/dev7/leet/smi64_atoms32_alphabet50_DM.npy')
