...
def create_dataset(conf):
    ...
    dataset = MoleculeDataset_Int(conf.input_file_SmilesEnc, conf.input_file_DM, conf.input_file_cheminfo,
                                  conf.input_file_dist_first,conf.input_file_fullDM,conf.input_file_conformation,100,conf.conformer_blinding_rate)  # MoleculeDataset_Int('C:/dev7/leet/smi64_atoms32_alphabet50_SmilesEncoded.npy', 'C:/dev7/leet/smi64_atoms32_alphabet50_DM.npy')
...
def train(model, dataloader, optimizer, criterion, device, dist_matrix_parts, num_batches_per_minibatch):
    model.train()
    total_loss_dm = 0
    batchcnt = 0
    ...
    for i, batch in enumerate(dataloader):
        ...
        inputs, targets_full_matrix = batch[0].to(device), batch[4].to(device)
        ...
        outputs_fulldm = outputs_all[1].permute(0, 2, 1)
        batchcnt = batchcnt + 1

        output_dm_reshaped = outputs_fulldm.view(-1, 17, 32, 64)[:, :, dist_matrix_parts, 0:32]
        targets_full_matrix_reshaped = targets_full_matrix.view(-1, 32, 32)[:, dist_matrix_parts, :]
        targets_full_matrix_reshaped = torch.clamp_max(targets_full_matrix_reshaped,8)
        loss_dm = criterion(output_dm_reshaped, targets_full_matrix_reshaped)
    ...
    return total_loss_dm / len(dataloader)

def validate(model, dataloader, criterion, device, dist_matrix_parts):
    model.eval()
    total_loss_dm = 0
    correct_preds = 0
    ...
    return ( (total_loss_dm) / len(dataloader) , correct_preds)
...
model = Seq2SeqModel_4(vocab_size_in=45,expansions_list=expansions,model_dim=conf.model_dim, num_layers=conf.model_layers) # (B)
...
for epoch in range(num_epochs):
    train_loss = train(model, train_loader, optimizer, criterion, device, adjacency_matrix_parts, conf.optim_batches_per_minibatch)
    val_loss = validate(model, val_loader, criterion, device, adjacency_matrix_parts)
    scheduler.step()

    print(
        f'Epoch: {epoch + 1}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss[0]:.4f}, Fully correct: {val_loss[1]} / {len(val_loader.dataset)}')
